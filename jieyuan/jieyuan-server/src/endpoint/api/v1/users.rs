...
use hetus::web::{WebError, WebResult, extract_ctx, ok_json};
...
#[utoipa::path(
  ...
  responses(
    (status = 200, description = "查询成功", body = hetusql::page::PageResult<User>),
    (status = 400, description = "请求参数错误")
  ),
  tag = "用户管理"
)]
async fn user_page(user_svc: UserSvc, Json(req): Json<UserForPage>) -> WebResult<hetusql::page::PageResult<User>> {
...
async fn user_update_password(
  ...
  // 从请求中提取用户上下文
  let ctx = extract_ctx(&parts, _app.hetu_setting().security())?;
