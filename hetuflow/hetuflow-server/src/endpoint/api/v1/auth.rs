...
fn is_localhost(addr: &SocketAddr) -> bool {
  match addr.ip() {
    ...
    IpAddr::V6(ipv6) => ipv6 == std::net::Ipv6Addr::LOCALHOST,
  }
}
...
pub async fn generate_token(
  ...
  if !is_localhost(&addr) {
    return Err((
      ...
      Json(ErrorResponse {
        ...
        message: "此接口仅允许本机访问".to_string(),
        details: Some(format!("访问来源: {}", addr.ip())),
      }),
    ));
  }
  ...
  let jwe_config = app.setting().jwe.as_ref().ok_or_else(|| {
    (
      ...
      Json(ErrorResponse {
        code: "JWE_NOT_CONFIGURED".to_string(),
        message: "JWE 认证未配置".to_string(),
        details: None,
      }),
    )
  })?;
  ...
  let jwe_service = JweService::new(jwe_config.clone()).map_err(|e| {
    (
      StatusCode::INTERNAL_SERVER_ERROR,
      Json(ErrorResponse {
        code: "JWE_SERVICE_ERROR".to_string(),
        message: "JWE 服务初始化失败".to_string(),
