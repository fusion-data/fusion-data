...
use hetuflow_core::protocol::{GatewayCommand, WebSocketCommand, WebSocketEvent, WebSocketParams};
...
use crate::{application::ServerApplication, gateway::MessageHandler, model::AgentConnection};
...
pub async fn handle_websocket_connection(
  ...
  let (agent_tx, mut agent_rx) = mpsc::unbounded_channel::<WebSocketCommand>();
  let agent_connection = AgentConnection::new(agent_id, address, agent_tx.clone());
  if let Err(e) = message_handler.add_connection(agent_id, agent_connection).await {
    error!("Failed to add agent connection: {:?}", e);
    return;
  }

  // 启动一个独立的任务，负责从 MPSC（agent_rx） 通道接收消息并发送到 WebSocket
