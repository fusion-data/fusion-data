//! Moonshot V1 最小端到端驱动示例（kimi-k2-0905-preview）
//!
//! 运行前请确保环境变量 `MOONSHOT_API_KEY` 已设置或在 .env 文件中声明。
//! 运行命令：
//!   cargo run -p hetumind-nodes --example moonshot_smoke
//!
//! 本示例构建一个最小的 Workflow/NodeExecutionContext，并调用 MoonshotV1 节点的 execute 方法，
//! 以验证系统提示、历史消息（用户侧）和参数绑定（temperature/max_tokens）是否生效，并打印统一的 used_params/history_length。
...
use hetumind_nodes::llm::moonshot_node::MoonshotModelNode;
use serde_json::json;
use uuid::Uuid;
...
fn build_input_json() -> JsonValue {
  json!({
    ...
    "messages": [
      {"role": "user", "content": "讲解 Rust 所有权与借用的区别。"},
      {"role": "assistant", "content": "当然，我来说明。"},
      {"role": "user", "content": "并对比 C++ 的做法。"}
    ],
    "temperature": 0.7,
    "max_tokens": 512
  })
...
/// 构造最小节点参数（MoonshotV1 节点配置）
fn build_node_parameters() -> hetumind_core::workflow::ParameterMap {
  let mut map = serde_json::Map::new();
  map.insert("model".to_string(), json!("kimi-k2-0905-preview"));
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  let mut workflow = Workflow::new(workflow_id, "moonshot_smoke");

  let node_kind = NodeKind::new(hetumind_nodes::constants::MOONSHOT_MODEL_NODE_KIND);
  let node_name = NodeName::from("moonshot_v1");
  ...
  // 4) 执行 MoonshotV1 节点
  let model_node = MoonshotModelNode::new().expect("create moonshot model node");
  let executor = model_node.node_executors().first().expect("moonshot v1 executor").clone();
