//! OpenAI V1 最小端到端驱动示例（gpt-4o-mini）
//!
//! 运行前请确保环境变量 `OPENAI_API_KEY` 已设置或在 .env 文件中声明。
//! 运行命令：
//!   cargo run -p hetumind-nodes --example openai_smoke
//!
//! 本示例构建一个最小的 Workflow/NodeExecutionContext，并调用 OpenaiV1 节点的 execute 方法，
//! 以验证系统提示、历史消息（用户侧）和参数绑定（temperature/max_tokens/top_p/stop）是否生效，并打印统一的 used_params/history_length。
...
use hetumind_nodes::llm::openai_node::OpenaiModelNode;
use serde_json::json;
use uuid::Uuid;
...
fn build_input_json() -> JsonValue {
  json!({
    "system_prompt": "你是一个专业助理，请简洁准确回答。",
    "messages": [
      {"role": "user", "content": "解释 Rust 所有权与借用的核心规则。"},
      {"role": "assistant", "content": "当然，我来说明。"},
      {"role": "user", "content": "并给出与 GC 的对比。"}
    ],
    "temperature": 0.7,
    "max_tokens": 512,
    "top_p": 0.9,
...
/// 构造最小节点参数（OpenaiV1 节点配置）
fn build_node_parameters() -> hetumind_core::workflow::ParameterMap {
  let mut map = serde_json::Map::new();
  map.insert("model".to_string(), json!("gpt-4o-mini"));
  map.insert("max_tokens".to_string(), json!(1024));
  map.insert("temperature".to_string(), json!(0.7));
  map.insert("top_p".to_string(), json!(0.9));
  map.insert("stop_sequences".to_string(), json!(vec!["### END"]));
  map.insert(
    "common".to_string(),
    json!({
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  let mut workflow = Workflow::new(workflow_id, "openai_smoke");

  let node_kind = NodeKind::new(hetumind_nodes::constants::OPENAI_MODEL_NODE_KIND);
  let node_name = NodeName::from("openai_v1");
  ...
  // 4) 执行 OpenaiV1 节点
  let model_node = OpenaiModelNode::new().expect("create openai model node");
  let executor = model_node.node_executors().first().expect("openai v1 executor").clone();
