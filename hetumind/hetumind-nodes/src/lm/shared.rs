...
use hetumind_core::{
  ...
  version::Version,
  workflow::{
    ExecutionData, ExecutionDataItems, ExecutionDataMap, InputPortConfig, NodeConnectionKind, NodeDescription,
    NodeExecutionContext, NodeExecutionError, NodeGroupKind, NodeName, NodeProperty, NodePropertyKind, NodeType,
    OutputPortConfig,
  },
};
use rig::{
...
pub fn create_base_node_definition(
  ...
  NodeDescription::new(node_type, description)
    .with_version(Version::new(1, 0, 0))
    .with_description(format!("{} ({})", description, model_provider))
    .add_group(NodeGroupKind::Transform)
    .with_icon("🧠")
    // Input port for messages
    .add_input(InputPortConfig::new(NodeConnectionKind::Main, "聊天消息输入")
        .with_required(true))
    // Output ports
    .add_output(OutputPortConfig::new(NodeConnectionKind::Main, "模型响应"))
    .add_output(OutputPortConfig::new(NodeConnectionKind::AiLanguageModel, "模型实例"))
...
pub fn create_llm_execution_data_map(
  ...
  map.insert(
    ...
    vec![ExecutionDataItems::Items(vec![ExecutionData::new_json(model_info, None)])],
  );
