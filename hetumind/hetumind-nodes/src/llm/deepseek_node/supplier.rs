...
use hetumind_core::workflow::{
  LLMResponse, LLMSubNodeProvider, Message, NodeDefinition, NodeExecutionError, NodeGroupKind, SubNode, SubNodeType,
};
use rig::{
  OneOrMany,
...
/// 当前实现为占位版本，后续将接入 rig-core 的 Agent/Client 完成真实调用
pub struct DeepseekModelSupplier {
  definition: Arc<NodeDefinition>,
}

impl DeepseekModelSupplier {
  fn create_definition() -> NodeDefinition {
    NodeDefinition::new(DEEPSEEK_MODEL_NODE_KIND, "DeepSeek Model Supplier")
      .with_version(Version::new(1, 0, 0))
      .add_group(NodeGroupKind::Transform)
      .with_description("Supply DeepSeek LLM provider for Agent consumption")
      .add_output(hetumind_core::workflow::OutputPortConfig::new(
        hetumind_core::workflow::ConnectionKind::AiLM,
        "Model",
      ))
      .add_output(hetumind_core::workflow::OutputPortConfig::new(
...
impl Default for DeepseekModelSupplier {
  fn default() -> Self {
    Self::new()
  }
}
...
impl LLMSubNodeProvider for DeepseekModelSupplier {
  ...
  async fn call_llm(
    ...
    let model_name = _config.model.clone();
    let mut ab = client.agent(&model_name);
    // 绑定参数：temperature / max_tokens
    if let Some(t) = _config.temperature {
      ab = ab.temperature(t);
    }
    if let Some(mt) = _config.max_tokens.map(|v| v as u64) {
      ab = ab.max_tokens(mt);
    }
    // 透传 top_p 与 stop（OpenAI 兼容字段名），采用 additional_params
    let mut extra = serde_json::Map::new();
    if let Some(tp) = _config.top_p.map(|v| v as f64) {
      extra.insert("top_p".to_string(), json!(tp));
    }
    if let Some(stops) = _config.stop_sequences.as_ref()
      && !stops.is_empty()
    {
