#[cfg(test)]
mod tests {
  use hetumind_core::workflow::{ConnectionKind, NodeGroupKind, Node};

  use crate::core::LimitNode;
  use crate::core::limit_node::{LimitConfig, KeepStrategy};
  ...
  fn test_limit_config_validation() {
    ...
    // 无效配置 - max_items 为 0
    let invalid_config = LimitConfig {
      max_items: 0,
      keep_strategy: KeepStrategy::FirstItems,
      warn_on_limit: true,
    };
