...
impl TryFrom<NodeDefinition> for ChatModelV1 {
  ...
  fn try_from(base: NodeDefinition) -> Result<Self, Self::Error> {
    let definition = base
      ...
      // 模型实例端口
      .add_output(OutputPortConfig::new(ConnectionKind::AiModel, "模型实例"))
      // 错误输出端口
      .add_output(OutputPortConfig::new(ConnectionKind::Error, "错误输出"))
      // 参数
      .add_property(NodeProperty::new(NodePropertyKind::String)
            .with_name("provider")
            .with_display_name("模型提供者")
            .with_value(json!("openai"))
            .with_required(true))
      .add_property(NodeProperty::new(NodePropertyKind::String)
            .with_name("model")
            .with_display_name("模型名称")
            .with_value(json!("gpt-3.5-turbo"))
            .with_required(true))
          .add_property(NodeProperty::new(NodePropertyKind::String)
            .with_name("credential_id")
            .with_display_name("凭证ID")
            .with_description("用于获取API密钥的凭证ID，如果提供则优先使用凭证服务")
            .with_required(false))
          .add_property(NodeProperty::new(NodePropertyKind::String)
            .with_name("api_key")
            .with_display_name("API 密钥")
            .with_description("API密钥，当未指定凭证ID时使用")
            .with_required(false))
          .add_property(NodeProperty::new(NodePropertyKind::String)
            .with_name("base_url")
            .with_display_name("API 基础URL")
            .with_required(false))
          .add_property(NodeProperty::new(NodePropertyKind::Number)
            .with_name("max_tokens")
            .with_display_name("最大令牌数")
            .with_value(json!(2000))
            .with_required(false))
          .add_property(NodeProperty::new(NodePropertyKind::Number)
