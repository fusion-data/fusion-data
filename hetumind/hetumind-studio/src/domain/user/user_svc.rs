...
impl UserSvc {
  pub async fn find_page(&self, for_query: UserForPage) -> Result<PageResult<UserEntity>, DataError> {
    UserBmc::page(self.ctx.mm(), vec![for_query.filter], for_query.page).await.map_err(DataError::from)
  }
  ...
  pub async fn create(&self, entity_c: UserForCreate) -> Result<i64, DataError> {
    ...
    UserBmc::create(self.ctx.mm(), entity_c).await.map_err(DataError::from)
  }

  pub async fn update(&self, id: i64, entity_u: UserForUpdate) -> Result<(), DataError> {
    UserBmc::update_by_id(self.ctx.mm(), id, entity_u).await.map_err(DataError::from)
  }

  pub async fn update_password(&self, id: i64, entity_u: UserForUpdatePassword) -> Result<(), DataError> {
    let user = UserBmc::find_by_id(self.ctx.mm(), id).await.map_err(DataError::from)?;
    ...
    UserBmc::set_new_password(self.ctx.mm(), id, password).await.map_err(DataError::from)
  }

  pub async fn delete(&self, id: i64) -> Result<(), DataError> {
    UserBmc::delete_by_id(self.ctx.mm(), id).await.map_err(DataError::from)
  }

  pub async fn get_by_id(&self, id: i64) -> Result<Option<UserEntity>, DataError> {
    UserBmc::get_by_id(self.ctx.mm(), id).await.map_err(DataError::from)
  }

  pub async fn find_by_id(&self, id: i64) -> Result<UserEntity, DataError> {
    UserBmc::find_by_id(self.ctx.mm(), id).await.map_err(DataError::from)
  }
}
