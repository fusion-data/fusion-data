  ...
  pub async fn route_engine_requests(
    ...
    if let Some(items) = output_data.get(&NodeConnectionKind::AiTool) {
            ...
            match serde_json::from_value::<EngineRequest>(json_val) {
              Ok(req) => {
                ...
                if let Some(tool_node) = workflow.get_node(&NodeName::from(req.node_name.as_str())) {
                  ...
                  input_map.insert(NodeConnectionKind::Main, vec![ExecutionDataItems::new_item(input_item)]);

                  // 创建 Tool 节点上下文并执行
                  let binary_data_manager =
                    Application::global().get_component::<BinaryDataManager>().map_err(|e| {
                      WorkflowExecutionError::InternalError { message: format!("BinaryDataManager not found: {}", e) }
                    })?;
                    ...
                    loop {
                      match tool_executor.execute(&tool_context).await {
                        Ok(tool_outputs) => {
                          ...
                          if let Some(tool_items) = tool_outputs.get(&NodeConnectionKind::Main)
                            && let Some(v) = tool_items.first().and_then(|x| x.get_data_items())
                          ...
                          if let Some(session_id) = req.metadata.get("session_id").and_then(|v| v.as_str())
                            && let Some(mem_supplier) = get_simple_memory_supplier_typed(&self.node_registry)
                          {
      ...
      if !responses.is_empty() {
        output_data.insert(NodeConnectionKind::AiTool, vec![ExecutionDataItems::new_items(responses)]);
      }
    }
