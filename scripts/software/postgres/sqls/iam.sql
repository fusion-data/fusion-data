...
create table if not exists iam.user (
  ...
  email varchar constraint user_uk_email unique,
  phone varchar constraint user_uk_phone unique,
  name varchar,
  status int not null,
  gender int not null,
  cid bigint not null,
...
create table if not exists iam.user_credential (
  id bigint not null constraint user_credential_fk_user references iam.user (id),
...
create table if not exists iam.permission (
  id bigserial not null,
  code varchar(255) not null constraint permission_uk unique,
  description text,
