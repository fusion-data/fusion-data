...
-- 初始化租户
insert into
  iam.tenant (name, cid, ctime)
values
  ('默认租户', 1, current_timestamp);

-- 初始化用户
insert into
  iam."user" (id, email, phone, name, status, gender, cid, ctime)
values
  (1, 'admin@fusiondata.com', null, '超管', 100, 0, 1, current_timestamp),
  (
    10000,
    'user@fusiondata.com',
    '13912345678',
...
insert into
  iam.user_credential (id, encrypted_pwd, cid, ctime)
values
  (
    1,
...
insert into
  iam.user_tenant_rel (user_id, tenant_id, cid, ctime)
values
...
-- 初始化数据
insert into
  iam.role (id, name, description, status, cid, ctime)
values
  (1, '超级管理员', '拥有所有权限的角色', 100, 1, current_timestamp),
  (2, '普通用户', '基本权限的角色', 100, 1, current_timestamp);

insert into
  iam.permission (id, code, description, resource, action, cid, ctime)
values
  (1, '用户查看', '查看用户信息的权限', 'user', 'read', 1, current_timestamp),
...
-- 为超级管理员分配所有权限
insert into
  iam.role_permission (role_id, permission_id, cid, ctime)
values
  (1, 1, 1, current_timestamp),
...
-- 为现有用户分配角色
insert into
  iam.user_role (user_id, role_id, cid, ctime)
values
  (1, 1, 1, current_timestamp), -- 超管用户分配超级管理员角色
