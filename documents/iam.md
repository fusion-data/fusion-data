...
- 约束与约定：
  ...
  - 数据库访问统一使用 BMC 模式（`hetusql` 宏），并复用当前项目已有模块与错误处理：`hetu-common`、`hetu-core`、`hetu-db`、`hetu-web`、`hetusql`、`jieyuan-core`。
    - BMC 参考示例见 `jieyuan/jieyuan-server/src/namespace/namespace_bmc.rs` 和 `jieyuan/jieyuan-server/src/user/user_credential_bmc.rs`
  - 错误统一复用：
    - 业务及通用错误：`hetu_core::DataError`
    - BMC 层错误：`hetusql::SqlError`
    - Axum Route 错误：`hetu_web::WebError`
...
条件键的数据来源于 `hetu_common::ctx::Ctx` 通过 `CtxExt` trait 提供：
...
```rust
use std::sync::Arc;
use hetu_core::{Result, DataError};
...
///   `.hetu_setting().app().time_offset()`；业务层由调用方传入所需配置，不直接访问 `Application::global()`。
/// - 占位符来源统一：模板渲染的内置占位符仅来自 `Ctx`；路由参数统一通过 `extras` 显式注入。
/// - 便捷构建方法统一：直接使用 `hetu_common::ctx::Ctx`，无需额外转换。
/// - 错误分层统一：仓库层返回 `hetu_core::DataError`（内部映射 `SqlError`）；端点/中间件仅使用 `hetu_web::WebError`。
#[derive(Clone)]
...
```rust
use hetusql::{base::DbBmc, generate_pg_bmc_common, generate_pg_bmc_filter};
...
## 中间件与路由集成（Axum + hetu-web）

- 在路由层引入鉴权中间件，统一执行授权：

```rust
use axum::{Router, routing::get};
use hetu_web::auth::{AsyncRequireAuthorizationLayer, WebAuth};
...
use hetus::common::ctx::Ctx;
use hetu_web::WebError;
use hetu_core::application::Application;
use hetu_core::model::{auth_ctx::build_auth_context, PolicySvc};
...
pub async fn authz_guard<B>(
  ...
  let ac = build_auth_context(&ctx, *app.hetu_setting().app().time_offset())
...
为满足“jieyuan 作为独立 IAM 微服务，其他项目仅通过远程调用使用”的约束，定义统一的远程授权接口。客户端（hetumind-studio、hetuflow-server）通过该接口完成令牌校验与权限评估，并复用既有数据结构：返回中的 `decision` 使用 `DecisionEffect`（值为 `allow`/`deny`），失败错误信息复用 `hetu_web::WebError`。
...
- 403 Forbidden（权限拒绝）：
  - 语义：令牌有效，但策略评估拒绝。
  - 结构：错误信息复用 `hetu_web::WebError`，并在 `err_detail` 中携带 `decision=deny` 与 `ctx`（便于客户端日志与提示）。
  - 示例：
    ```json
    {
      "err_code": 403,
...
- 401 Unauthorized（令牌无效或过期）：
  ...
  - 结构：错误信息复用 `hetu_web::WebError`；不返回 `ctx`（不可用）。
  - 示例：
    ```json
    {
      "err_code": 401,
...
- 参考 `hetu-core` 配置注入方式，为 IAM 增加简单配置：
...
pub async fn authorize(
    ...
    // 2) 构建授权上下文 (Ctx)
    let time_offset = *app.hetu_setting().app().time_offset();
...
hetu-core = { workspace = true, features = ["with-uuid"] }
hetu-web = { workspace = true, features = ["with-uuid"] }
axum = { workspace = true }
...
- 统一错误类型（按层次使用）：
  - 业务服务层统一使用 `hetu_core::DataError`；
  - 数据库访问层统一使用/转换为 `hetusql::SqlError`；
  - 仅在 Axum 端点函数及中间件中使用 `hetu_web::WebError` 作为响应错误类型。
