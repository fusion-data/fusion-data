# 使用 @hetu-data/hetusql 包的示例

## 导入类型

```typescript
// 从 hetusql 导入分页和操作符类型
import { PageResult, OpValString, OpValNumber, OpValBool } from '@hetu-data/hetusql';
import { HetumindSDK } from '@hetu-data/hetumind-sdk';

// 在 hetumind-sdk 中使用 hetusql 类型
const sdk = new HetumindSDK({
  baseURL: 'http://localhost:3000',
  token: 'your-token',
});

// 使用 hetusql 的 OpValString 进行字符串过滤
const workflowQuery = {
...
// 使用 hetusql 的 OpValNumber 进行数字过滤
...
// 使用 hetusql 的 PageResult 处理分页结果
