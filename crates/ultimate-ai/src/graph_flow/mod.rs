...
//!
//! ```rust
//! use ultimate_ai::graph_flow::{Context, Task, TaskResult, NextAction, GraphBuilder, FlowRunner, InMemorySessionStorage, Session, SessionStorage};
...
//!
//! # #[tokio::main]
//! # async fn main() -> ultimate_ai::graph_flow::Result<()> {
//! // Build the workflow
...
//!
//! ```rust
//! # use ultimate_ai::graph_flow::{GraphBuilder, Task, TaskResult, NextAction, Context};
//! # use async_trait::async_trait;
//! # use std::sync::Arc;
//! # struct Task1; struct Task2; struct Task3;
//! # #[async_trait] impl Task for Task1 { fn id(&self) -> &str { "task1" } async fn run(&self, _: Context) -> ultimate_ai::graph_flow::Result<TaskResult> { Ok(TaskResult::new(None, NextAction::End)) } }
//! # #[async_trait] impl Task for Task2 { fn id(&self) -> &str { "task2" } async fn run(&self, _: Context) -> ultimate_ai::graph_flow::Result<TaskResult> { Ok(TaskResult::new(None, NextAction::End)) } }
//! # #[async_trait] impl Task for Task3 { fn id(&self) -> &str { "task3" } async fn run(&self, _: Context) -> ultimate_ai::graph_flow::Result<TaskResult> { Ok(TaskResult::new(None, NextAction::End)) } }
...
//!
//! ```rust,no_run
//! # use ultimate_ai::graph_flow::{FlowRunner, InMemorySessionStorage, Session, Graph, SessionStorage};
//! # use std::sync::Arc;
//! # #[tokio::main]
//! # async fn main() -> ultimate_ai::graph_flow::Result<()> {
