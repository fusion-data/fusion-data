...
use ultimatesql_core::filter::{FilterNodes, OpValInt64, OpValString};
use ultimatesql_core::sea_utils::SIden;

#[derive(Clone, FilterNodes, Default)]
pub struct ProjectFilter {
  id: Option<OpValInt64>,
  name: Option<OpValString>,
  #[ultimatesql(rel = "foo_rel")]
  label: Option<OpValString>,
}

#[derive(Clone, FilterNodes, Default)]
#[ultimatesql(rel = "task_tbl")]
...
fn test_expand_filter_nodes_filter_rel() -> Result<()> {
  ...
  // -- Exec
  let cond: Result<sea_query::Condition, ultimatesql_core::filter::IntoSeaError> = filter.try_into();
  let cond = cond?;
