...
> hetu-db 是 Fusion-Data 项目的数据库层封装，提供 `ModelManager` 和 `DbPlugin`，集成 hetusql ORM。
...
use hetu_core::ApplicationBuilder;
...
hetu-db 重导出 hetusql 的核心类型：

```rust
use hetu_db::DbConfig;
use hetusql::{ModelManager, DbBmc, BmcConfig, SqlError};
```
...
```rust
use hetu_core::{Application, ApplicationBuilder, logforth::LogforthPlugin};
use hetu_db::DbPlugin;
