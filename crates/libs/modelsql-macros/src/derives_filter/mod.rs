...
use crate::utils::struct_modql_attr::get_struct_modelsql_props;
...
pub fn derive_filter_nodes_inner(input: TokenStream) -> TokenStream {
  ...
  let struct_attrs = get_struct_modelsql_props(&ast).unwrap();
    ...
    if type_name.starts_with("Option ") && type_name.contains("OpVal") {
      if let Some(ident) = field.ident.as_ref() {
        ...
        props_filter_node_options.push(quote! {
          modelsql::filter::FilterNodeOptions {
        ...
        let block = if let Some(to_sea_condition_fn) = modql_field_attr.to_sea_condition_fn {
          ...
          quote! {
            // None
            let fn_holder = modelsql::filter::ToSeaConditionFnHolder::new(#to_sea_condition_fn);
          ...
          quote! {
            // None
            let fn_holder = modelsql::filter::ToSeaValueFnHolder::new(#to_sea_value_fn);
            let fn_holder = Some(fn_holder.into());
          }
        } else {
          quote! {
  ...
  let ff_opt_node_pushes = quote! {
    #(
      if let Some(val) = self.#props_opval_idents {
        let op_vals: Vec<modelsql::filter::OpVal> = val.0.into_iter().map(|n| n.into()).collect();
        #props_opval_to_sea_holder_fn_build
        let node = modelsql::filter::FilterNode{
  ...
  let out_impl_into_filter_nodes = quote! {
    impl modelsql::filter::IntoFilterNodes for #struct_name {
      fn filter_nodes(self, rel: Option<String>) -> Vec<modelsql::filter::FilterNode> {
  ...
  let out_into_filter_node = quote! {
    impl From<#struct_name> for Vec<modelsql::filter::FilterNode> {
      fn from(val: #struct_name) -> Self {
        modelsql::filter::IntoFilterNodes::filter_nodes(val, None)
      }
    }
  };

  let out_into_op_group = quote! {
    impl From<#struct_name> for modelsql::filter::FilterGroup {
      fn from(val: #struct_name) -> Self {
        let nodes: Vec<modelsql::filter::FilterNode> = val.into();
        nodes.into()
      }
    }
  };
  ...
  let out_into_op_groups = quote! {
    impl From<#struct_name> for modelsql::filter::FilterGroups {
      fn from(val: #struct_name) -> Self {
        let nodes: Vec<modelsql::filter::FilterNode> = val.into();
        nodes.into()
      }
  ...
  let out_sea_filter = quote! {
    impl TryFrom<#struct_name> for sea_query::Condition {
      type Error = modelsql::filter::IntoSeaError;

      fn try_from(val: #struct_name) -> modelsql::filter::SeaResult<Self> {
        modelsql::filter::FilterGroup::from(val).try_into()
      }
    }
  };
