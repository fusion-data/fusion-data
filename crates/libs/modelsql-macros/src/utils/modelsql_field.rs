...
// pub fn get_modelsql_field_props(fields: &FieldsNamed) -> Vec<ModelsqlFieldProp> {
//   let modelsql_fields_and_skips = get_modelsql_field_props_and_skips(fields);
//   modelsql_fields_and_skips.modelsql_fields
// }

pub struct ModelsqlFieldsAndSkips<'a> {
  pub modelsql_fields: Vec<ModelsqlFieldProp<'a>>,
  #[allow(unused)] // For early development.
  pub skipped_fields: Vec<&'a Field>,
  pub field_mask_field: Option<&'a Field>,
}

pub fn get_modelsql_field_props_and_skips<'a>(fields: &'a FieldsNamed) -> ModelsqlFieldsAndSkips<'a> {
  ...
  for field in fields.named.iter() {
    ...
    // -- Add to array.
    modql_fields.push(ModelsqlFieldProp { rel: mfield_attr.rel, name, prop_name, attr_name, ident, cast_as, is_option })
  }

  ModelsqlFieldsAndSkips { modelsql_fields: modql_fields, skipped_fields, field_mask_field }
}
...
fn is_valid_field_mask_path(path: &syn::Path) -> bool {
  ...
  match segments.as_slice() {
    ...
    // 完整路径: modelsql::field::FieldMask
    [modelsql, field, field_mask] if *modelsql == "modelsql" && *field == "field" && *field_mask == "FieldMask" => true,
