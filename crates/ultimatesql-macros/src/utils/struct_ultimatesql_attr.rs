...
pub fn get_struct_ultimatesql_props(dinput: &DeriveInput) -> Result<StructModqlFieldProps, syn::Error> {
  // FIXME: We should remove this, 'sqlb' should not be a thing anymore.
  let sqlb_attr = get_input_attribute(dinput, "ultimatesql");
  let mut rel = None;
  ...
  if let Some(attribute) = sqlb_attr {
    ...
    for meta in nested {
      match meta {
        ...
        /* ... */
        _ => {
          let msg = "unrecognized ultimatesql attribute value";
