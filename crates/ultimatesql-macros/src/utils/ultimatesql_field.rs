...
// pub fn get_ultimatesql_field_props(fields: &FieldsNamed) -> Vec<ModelsqlFieldProp> {
//   let ultimatesql_fields_and_skips = get_ultimatesql_field_props_and_skips(fields);
//   ultimatesql_fields_and_skips.ultimatesql_fields
// }

pub struct ModelsqlFieldsAndSkips<'a> {
  pub ultimatesql_fields: Vec<ModelsqlFieldProp<'a>>,
  #[allow(unused)] // For early development.
  pub skipped_fields: Vec<&'a Field>,
  pub field_mask_field: Option<&'a Field>,
}

pub fn get_ultimatesql_field_props_and_skips<'a>(fields: &'a FieldsNamed) -> ModelsqlFieldsAndSkips<'a> {
  let mut ultimatesql_fields = Vec::new();
  ...
  for field in fields.named.iter() {
    ...
    // -- Add to array.
    ultimatesql_fields.push(ModelsqlFieldProp {
      rel: mfield_attr.rel,
      name,
      prop_name,
      attr_name,
  ...
  ModelsqlFieldsAndSkips { ultimatesql_fields, skipped_fields, field_mask_field }
}
...
fn is_valid_field_mask_path(path: &syn::Path) -> bool {
  ...
  match segments.as_slice() {
    ...
    // 完整路径: ultimatesql::field::FieldMask
    [ultimatesql, field, field_mask]
      if *ultimatesql == "ultimatesql" && *field == "field" && *field_mask == "FieldMask" =>
    {
      true
    }
