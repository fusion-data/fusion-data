...
use ultimate_db::modql::field::{Fields, HasFields, HasSeaFields};
use ultimate_db::modql::{SIden, SqliteFromRow};
...
fn test_sea_insert_and_raw_select() -> Result<()> {
  ...
  // -- Insert
  let agent_c =
    AgentForCreate { name: Some(fx_agent_name.to_string()), model: None, level: Some(fx_level), module_id: None };
