...
mod json {
  ...
  use crate::modql::filter::OpValValue;
...
mod with_sea_query {
  use super::*;
  use crate::modql::filter::{sea_is_col_value_null, FilterNodeOptions, SeaResult, ToSeaValueFnHolder};
  ...
  impl OpValValue {
    pub fn into_sea_cond_expr_with_json_to_sea(
      ...
      let binary_fn = |op: BinOper, json_value: serde_json::Value| -> SeaResult<ConditionExpression> {
        ...
        Ok(ConditionExpression::SimpleExpr(SimpleExpr::binary(col.clone().into(), op, vxpr)))
      };
