  ...
  fn to_tokens(&self, tokens: &mut TokenStream) {
    ...
    match ty {
      ...
      InjectableType::ConfigArc(type_path) => tokens.extend(quote! {
        #field_name: ::ultimate_core::config::ConfigArc::new(app.get_config::<#type_path>()?)
      }),
...
pub(crate) fn expand_derive(input: syn::DeriveInput) -> syn::Result<TokenStream> {
  ...
  let token_stream = quote! {
    impl ::ultimate_core::component::Component for #ident {
      fn build(app: &::ultimate_core::application::ApplicationBuilder) -> ::ultimate_core::Result<Self> {
    ...
    impl ::ultimate_core::component::ComponentInstaller for #component_registrar {
      fn dependencies(&self) -> Vec<&str> {
        vec![#(std::any::type_name::<#dependencies>()),*]
        // static_component_registrar.to_vec()
      }

      fn install_component(&self, app: &mut ::ultimate_core::application::ApplicationBuilder)->::ultimate_core::Result<()> {
        use ::ultimate_core::component::Component;
        app.add_component(#ident::build(app).unwrap());
        Ok(())
      }
    }
    ::ultimate_core::submit_component!(#component_registrar);
  };
