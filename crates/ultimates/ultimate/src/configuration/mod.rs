...
pub use ultimate_macros::Configuration;
...
impl UltimateConfigRegistry {
  ...
  pub fn add_config_source<T>(&self, source: T) -> Result<()>
  where
  ...
  pub fn get_config<T>(&self) -> crate::Result<T>
  where
    T: DeserializeOwned + Configurable,
  {
    let c = self.config.read().unwrap().get(T::config_prefix())?;
    Ok(c)
  }
}
