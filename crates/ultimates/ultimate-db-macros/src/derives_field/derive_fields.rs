...
fn impl_has_fields(
  ...
  let props_field_refs = field_props.iter().map(|field_prop| {
    ...
    quote! {&ultimate_db::modql::field::FieldRef{rel: #rel, name: #name}}
  });
  ...
  let props_field_metas = field_props.iter().map(|field_prop| {
    ...
    quote! {&ultimate_db::modql::field::FieldMeta{
        rel: #rel,
  ...
  let output = quote! {
    ...
    impl ultimate_db::modql::field::HasFields for #struct_name {
      ...
      fn field_refs() -> &'static [&'static ultimate_db::modql::field::FieldRef] {
        &[#(
        #props_field_refs,
        )*]
      }

      fn field_metas() -> &'static ultimate_db::modql::field::FieldMetas {
        static METAS: &[&ultimate_db::modql::field::FieldMeta] = &[#(
        #props_field_metas,
        )*];

        static METAS_HOLDER: ultimate_db::modql::field::FieldMetas = ultimate_db::modql::field::FieldMetas::new(METAS);
...
fn impl_has_sea_fields(
  ...
  fn field_options_quote(mfield_prop: &ModqlFieldProp) -> proc_macro2::TokenStream {
    if let Some(cast_as) = &mfield_prop.cast_as {
      quote! { ultimate_db::modql::field::FieldOptions { cast_as: Some(#cast_as.to_string()) } }
    } else {
      quote! { ultimate_db::modql::field::FieldOptions { cast_as: None } }
    }
  }
  ...
  let all_fields_quotes = field_props.iter().map(|p| {
    ...
    quote! {
      ff.push(
        ultimate_db::modql::field::SeaField::new_with_options(ultimate_db::modql::SIden(#name), self.#ident.into(), #field_options_q)
      );
    }
  });
    ...
    if p.is_option {
      quote! {
          if let Some(val) = self.#ident {
            ff.push(
              ultimate_db::modql::field::SeaField::new_with_options(ultimate_db::modql::SIden(#name), val.into(), #field_options_q)
            );
          }
      }
    } else {
  ...
  let output = quote! {
    ...
    impl ultimate_db::modql::field::HasSeaFields for #struct_name {

      fn not_none_sea_fields(self) -> ultimate_db::modql::field::SeaFields {
        let mut ff: Vec<ultimate_db::modql::field::SeaField> = Vec::new();
        #(#not_none_fields_quotes)*
        ultimate_db::modql::field::SeaFields::new(ff)
      }

      fn all_sea_fields(self) -> ultimate_db::modql::field::SeaFields {
        let mut ff: Vec<ultimate_db::modql::field::SeaField> = Vec::new();
        #(#all_fields_quotes)*
        ultimate_db::modql::field::SeaFields::new(ff)
      }
      ...
      fn sea_idens() -> Vec<sea_query::SeaRc<dyn sea_query::Iden>> {
        vec![#(
        sea_query::IntoIden::into_iden(ultimate_db::modql::SIden(#prop_all_names)),
        )*]
      }
