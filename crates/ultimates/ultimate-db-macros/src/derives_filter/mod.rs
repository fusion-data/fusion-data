...
pub fn derive_filter_nodes_inner(input: TokenStream) -> TokenStream {
    ...
    if type_name.starts_with("Option ") && type_name.contains("OpVal") {
      if let Some(ident) = field.ident.as_ref() {
        ...
        props_filter_node_options.push(quote! {
          ultimate_db::modql::filter::FilterNodeOptions {
        ...
        let block = if let Some(to_sea_condition_fn) = modql_field_attr.to_sea_condition_fn {
          ...
          quote! {
            // None
            let fn_holder = ultimate_db::modql::filter::ToSeaConditionFnHolder::new(#to_sea_condition_fn);
          ...
          quote! {
            // None
            let fn_holder = ultimate_db::modql::filter::ToSeaValueFnHolder::new(#to_sea_value_fn);
            let fn_holder = Some(fn_holder.into());
          }
        } else {
          quote! {
  ...
  let ff_opt_node_pushes = quote! {
    #(
      if let Some(val) = self.#props_opval_idents {
        let op_vals: Vec<ultimate_db::modql::filter::OpVal> = val.0.into_iter().map(|n| n.into()).collect();
        #props_opval_to_sea_holder_fn_build
        let node = ultimate_db::modql::filter::FilterNode{
  ...
  let out_impl_into_filter_nodes = quote! {
    impl ultimate_db::modql::filter::IntoFilterNodes for #struct_name {
      fn filter_nodes(self, rel: Option<String>) -> Vec<ultimate_db::modql::filter::FilterNode> {
  ...
  let out_into_filter_node = quote! {
    impl From<#struct_name> for Vec<ultimate_db::modql::filter::FilterNode> {
      fn from(val: #struct_name) -> Self {
        ultimate_db::modql::filter::IntoFilterNodes::filter_nodes(val, None)
      }
    }
  };

  let out_into_op_group = quote! {
    impl From<#struct_name> for ultimate_db::modql::filter::FilterGroup {
      fn from(val: #struct_name) -> Self {
        let nodes: Vec<ultimate_db::modql::filter::FilterNode> = val.into();
        nodes.into()
      }
    }
  };
  ...
  let out_into_op_groups = quote! {
    impl From<#struct_name> for ultimate_db::modql::filter::FilterGroups {
      fn from(val: #struct_name) -> Self {
        let nodes: Vec<ultimate_db::modql::filter::FilterNode> = val.into();
        nodes.into()
      }
  ...
  let out_sea_filter = quote! {
    impl TryFrom<#struct_name> for sea_query::Condition {
      type Error = ultimate_db::modql::filter::IntoSeaError;

      fn try_from(val: #struct_name) -> ultimate_db::modql::filter::SeaResult<Self> {
        ultimate_db::modql::filter::FilterGroup::from(val).try_into()
      }
    }
  };
