...
pub fn get_modql_field_props_and_skips(fields: &FieldsNamed) -> ModqlFieldsAndSkips {
  ...
  for field in fields.named.iter() {
    ...
    // -- Add to array.
    modql_fields.push(ModqlFieldProp { rel: mfield_attr.rel, name, prop_name, attr_name, ident, cast_as, is_option })
  }

  ModqlFieldsAndSkips { modql_fields, skipped_fields }
}
...
"#,
  ...
  Ok(ModqlFieldPropAttr { skip, rel, name: column, cast_as })
}
