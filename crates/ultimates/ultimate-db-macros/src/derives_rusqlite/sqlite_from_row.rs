...
pub fn derive_sqlite_from_row_inner(input: TokenStream) -> TokenStream {
  ...
  let ModqlFieldsAndSkips { modql_fields, skipped_fields } = modql_field::get_modql_field_props_and_skips(fields);
  ...
  let output = quote! {
    impl ultimate_db::modql::SqliteFromRow for #struct_name {
