...
// pub fn get_hetusql_field_props(fields: &FieldsNamed) -> Vec<ModelsqlFieldProp> {
//   let hetusql_fields_and_skips = get_hetusql_field_props_and_skips(fields);
//   hetusql_fields_and_skips.hetusql_fields
// }

pub struct ModelsqlFieldsAndSkips<'a> {
  pub hetusql_fields: Vec<ModelsqlFieldProp<'a>>,
  #[allow(unused)] // For early development.
  pub skipped_fields: Vec<&'a Field>,
  pub field_mask_field: Option<&'a Field>,
}

pub fn get_hetusql_field_props_and_skips<'a>(fields: &'a FieldsNamed) -> ModelsqlFieldsAndSkips<'a> {
  let mut hetusql_fields = Vec::new();
  ...
  for field in fields.named.iter() {
    ...
    // -- Add to array.
    hetusql_fields.push(ModelsqlFieldProp {
      rel: mfield_attr.rel,
      name,
      prop_name,
      attr_name,
  ...
  ModelsqlFieldsAndSkips { hetusql_fields, skipped_fields, field_mask_field }
}
