...
use crate::utils::struct_hetusql_attr::get_struct_hetusql_props;
...
pub fn derive_filter_nodes_inner(input: TokenStream) -> TokenStream {
  ...
  let struct_attrs = get_struct_hetusql_props(&ast).unwrap();
  ...
  for field in fields.named.iter() {
    ...
    if type_name.starts_with("Option ") && type_name.contains("OpVal") {
      if let Some(ident) = field.ident.as_ref() {
        ...
        let hetusql_field_attr = get_filter_field_attr(field).unwrap();

        // -- rel
        let block_rel = if let Some(rel) = hetusql_field_attr.rel {
          quote! {
            Some(#rel.to_string())
          }
        ...
        let quote_filter_node_options_cast_as = if let Some(cast_as) = hetusql_field_attr.cast_as {
          quote! { Some(#cast_as.to_string()) }
        } else {
          quote! { None }
        };
        props_filter_node_options.push(quote! {
          hetusql::filter::FilterNodeOptions {
        ...
        let block = if let Some(to_sea_condition_fn) = hetusql_field_attr.to_sea_condition_fn {
          let to_sea_condition_fn = syn::Ident::new(&to_sea_condition_fn, proc_macro2::Span::call_site());
          quote! {
            // None
            let fn_holder = hetusql::filter::ToSeaConditionFnHolder::new(#to_sea_condition_fn);
            let fn_holder = Some(fn_holder.into());
          }
        } else if let Some(to_sea_value_fn) = hetusql_field_attr.to_sea_value_fn {
          let to_sea_value_fn = syn::Ident::new(&to_sea_value_fn, proc_macro2::Span::call_site());
          quote! {
            // None
            let fn_holder = hetusql::filter::ToSeaValueFnHolder::new(#to_sea_value_fn);
            let fn_holder = Some(fn_holder.into());
          }
        } else {
          quote! {
  ...
  let ff_opt_node_pushes = quote! {
    #(
      if let Some(op_vals) = self.#props_opval_idents {
        // let op_vals: Vec<hetusql::filter::OpVal> = val.0.into_iter().map(|n| n.into()).collect();
        #props_opval_to_sea_holder_fn_build
        let node = hetusql::filter::FilterNode {
  ...
  let out_impl_into_filter_nodes = quote! {
    impl hetusql::filter::IntoFilterNodes for #struct_name {
      fn filter_nodes(self, rel: Option<String>) -> Vec<hetusql::filter::FilterNode> {
  ...
  let out_into_filter_node = quote! {
    impl From<#struct_name> for Vec<hetusql::filter::FilterNode> {
      fn from(val: #struct_name) -> Self {
        hetusql::filter::IntoFilterNodes::filter_nodes(val, None)
      }
    }
  };

  let out_into_op_group = quote! {
    impl From<#struct_name> for hetusql::filter::FilterGroup {
      fn from(val: #struct_name) -> Self {
        let nodes: Vec<hetusql::filter::FilterNode> = val.into();
        nodes.into()
      }
    }
  };
  ...
  let out_into_op_groups = quote! {
    impl From<#struct_name> for hetusql::filter::FilterGroups {
      fn from(val: #struct_name) -> Self {
        let nodes: Vec<hetusql::filter::FilterNode> = val.into();
        nodes.into()
      }
  ...
  let out_sea_filter = quote! {
    impl TryFrom<#struct_name> for sea_query::Condition {
      type Error = hetusql::filter::IntoSeaError;

      fn try_from(val: #struct_name) -> hetusql::filter::SeaResult<Self> {
        hetusql::filter::FilterGroup::from(val).try_into()
      }
    }
  };
