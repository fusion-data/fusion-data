...
pub(crate) fn derive_fields_inner(input: TokenStream) -> TokenStream {
  ...
  let hetusql_fields_and_skips = get_hetusql_field_props_and_skips(fields);
  let field_props = hetusql_fields_and_skips.hetusql_fields; //hetusql_field::get_hetusql_field_props(fields);
  let field_mask_field = hetusql_fields_and_skips.field_mask_field;

  let struct_hetusql_prop = get_struct_hetusql_props(&ast).unwrap();

  // Will be "" if none (this if for the struct #[hetusql(table = ...)])

  let impl_has_fields = impl_has_fields(struct_name, &struct_hetusql_prop, &field_props);

  let impl_names_as_consts = if let Some(names_as_consts) = struct_hetusql_prop.names_as_consts.as_deref() {
    //
  ...
  let impl_sea_fields = impl_has_sea_fields(struct_name, &struct_hetusql_prop, &field_props, field_mask_field);
...
fn impl_has_fields(
  struct_name: &Ident,
  struct_hetusql_prop: &StructModqlFieldProps,
  field_props: &[ModelsqlFieldProp<'_>],
) -> proc_macro2::TokenStream {
  let props_all_names: Vec<&String> = field_props.iter().map(|p| &p.name).collect();

  let struct_rel = struct_hetusql_prop.rel.as_ref();
  ...
  let props_field_refs = field_props.iter().map(|field_prop| {
    ...
    quote! {&hetusql::field::FieldRef{rel: #rel, name: #name}}
  });
  ...
  let output = quote! {
    ...
    impl hetusql::field::HasFields for #struct_name {
      ...
      fn field_refs() -> &'static [&'static hetusql::field::FieldRef] {
        &[#(
        #props_field_refs,
        )*]
      }

      fn field_metas() -> &'static hetusql::field::FieldMetas {
        static METAS: &[&hetusql::field::FieldMeta] = &[#(
        #props_field_metas,
        )*];

        static METAS_HOLDER: hetusql::field::FieldMetas = hetusql::field::FieldMetas::new(METAS);
...
fn impl_has_sea_fields(
  ...
  let prop_all_rels: Vec<String> = field_props
    ...
    .map(|p| {
      p.rel
        .as_ref()
        .map(|t| t.to_string())
        .unwrap_or_else(|| struct_hetusql_prop.rel.as_ref().map(|s| s.to_string()).unwrap_or_default())
    })
    .collect();

  fn field_options_quote(mfield_prop: &ModelsqlFieldProp) -> proc_macro2::TokenStream {
    if let Some(cast_as) = &mfield_prop.cast_as {
      quote! { hetusql::field::FieldOptions { cast_as: Some(#cast_as.to_string()) } }
    } else {
      quote! { hetusql::field::FieldOptions { cast_as: None } }
    }
  }
  ...
  let all_fields_quotes = field_props.iter().map(|p| {
    ...
    quote! {
      ff.push(
        hetusql::field::SeaField::new_with_options(hetusql::SIden(#name), self.#ident.into(), #field_options_q)
      );
    }
  });
    ...
    if p.is_option {
      quote! {
        if let Some(val) = self.#ident {
          ff.push(
            hetusql::field::SeaField::new_with_options(hetusql::SIden(#name), val.into(), #field_options_q)
          );
        }
      }
    } else {
  ...
  let sea_fields_with_mask_quotes = field_props.iter().map(|p| {
    ...
    let create_field_expr = |value_expr: proc_macro2::TokenStream| {
      quote! {
        ff.push(
          hetusql::field::SeaField::new_with_options(hetusql::SIden(#name), #value_expr.into(), #field_options_q)
        );
      }
    };
  ...
  let output = quote! {
    ...
    impl hetusql::field::HasSeaFields for #struct_name {

      fn not_none_sea_fields(self) -> hetusql::field::SeaFields {
        let mut ff: Vec<hetusql::field::SeaField> = Vec::new();
        #(#not_none_fields_quotes)*
        hetusql::field::SeaFields::new(ff)
      }

      fn sea_fields_with_mask(self) -> hetusql::field::SeaFields {
        let mut ff: Vec<hetusql::field::SeaField> = Vec::new();
        #(#sea_fields_with_mask_quotes)*
        hetusql::field::SeaFields::new(ff)
      }

      fn all_sea_fields(self) -> hetusql::field::SeaFields {
        let mut ff: Vec<hetusql::field::SeaField> = Vec::new();
        #(#all_fields_quotes)*
        hetusql::field::SeaFields::new(ff)
      }
      ...
      fn sea_idens() -> Vec<sea_query::SeaRc<dyn sea_query::Iden>> {
        vec![#(
        sea_query::IntoIden::into_iden(hetusql::SIden(#prop_all_names)),
        )*]
      }
