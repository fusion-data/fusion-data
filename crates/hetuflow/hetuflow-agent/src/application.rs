...
pub struct AgentApplication {
  ...
  shutdown_tx: Arc<Mutex<Option<(ShutdownSend, ShutdownRecv)>>>,
...
impl AgentApplication {
    ...
    S: config::Source + Send + Sync + 'static,
  {
    ...
    let process_manager =
      Arc::new(ProcessManager::new(setting.process.clone(), shutdown_rx.clone(), connection_manager.clone()));
    ...
    Ok(Self {
      ...
      process_manager,
      task_executor,
      shutdown_tx: Arc::new(Mutex::new(Some((shutdown_tx, shutdown_rx)))),
  ...
  pub async fn start(&self) -> Result<(), DataError> {
    ...
    handles.extend(self.process_manager.start()?);
    handles.extend(self.task_executor.start()?);
    handles.extend(self.task_scheduler.start()?);
    handles.extend(self.connection_manager.start()?);
  ...
  pub async fn shutdown(self) -> Result<(), DataError> {
    ...
    // 取出 ShutdownSend
    let shutdown_tx = match self.shutdown_tx.lock().await.take() {
