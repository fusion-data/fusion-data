...
pub fn make_token(sc: &SecuritySetting, payload: CtxPayload) -> hetu_core::Result<String> {
  let token =
    SecurityUtils::encrypt_jwt(sc.pwd(), payload).map_err(|_e| DataError::unauthorized("Failed generate token"))?;
  Ok(token)
}

pub fn make_token_by_user_id(sc: &SecuritySetting, uid: impl Into<String>) -> hetu_core::Result<String> {
