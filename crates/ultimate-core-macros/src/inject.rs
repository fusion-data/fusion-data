  ...
  fn to_tokens(&self, tokens: &mut TokenStream) {
    ...
    match ty {
      ...
      InjectableType::ConfigArc(type_path) => tokens.extend(quote! {
        #field_name: ::ultimates::core::config::ConfigArc::new(app.get_config::<#type_path>()?)
      }),
...
pub(crate) fn expand_derive(input: syn::DeriveInput) -> syn::Result<TokenStream> {
  ...
  let token_stream = quote! {
    impl ::ultimates::core::component::Component for #ident {
      fn build(app: &::ultimates::core::application::ApplicationBuilder) -> ::ultimates::core::Result<Self> {
        use ::ultimates::core::configuration::ConfigRegistry;
        Ok(#component)
      }
    }
    ...
    impl ::ultimates::core::component::ComponentInstaller for #component_registrar {
      fn dependencies(&self) -> Vec<&str> {
        vec![#(std::any::type_name::<#dependencies>()),*]
        // static_component_registrar.to_vec()
      }

      fn install_component(&self, app: &mut ::ultimates::core::application::ApplicationBuilder)->::ultimates::core::Result<()> {
        use ::ultimates::core::component::Component;
        app.add_component(#ident::build(app).unwrap());
        Ok(())
      }
    }
    ::ultimates::core::submit_component!(#component_registrar);
  };
