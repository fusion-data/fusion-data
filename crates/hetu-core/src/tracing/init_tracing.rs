...
pub(crate) fn init_tracing_guard() -> (DefaultGuard, Option<String>) {
  let c = LogSetting {
    ...
    log_dir: std::option_env!("HETU__LOG_DIR").unwrap_or_else(|| "./var/logs/").to_string(),
    ..Default::default()
  };
...
fn temporary_app_name() -> String {
  std::env::var("HETU__APP__NAME")
    .or_else(|_| std::env::var("HETU_APP_NAME"))
    .unwrap_or_else(|_| "hetu".to_string())
}

pub fn init_subscribers(setting: &HetuSetting) -> Result<Option<Guard>> {
