...
pub fn load_config_with(custom_config: Option<Config>) -> ConfigureResult<Config> {
  ...
  let profile_files = if let Ok(profiles_active) = env::var("HETU__PROFILES__ACTIVE") {
    vec![
  ...
  if let Ok(file) = get_env("HETU_CONFIG_FILE") {
    log::info!("Load config from env HETU_CONFIG_FILE: {}", file);
    let path = Path::new(&file);
    if path.exists() {
      log::info!("HETU_CONFIG_FILE exists, load config from path: {}", path.display());
