...
use config::Config;
use dashmap::DashMap;
use hetu_common::ahash::HashSet;
use hetu_common::time::OffsetDateTime;
use log::{debug, info};
use mea::{
  mutex::Mutex,
...
use crate::{
  ...
  configuration::{ConfigRegistry, Configurable, ConfigureResult, HetuConfigRegistry, HetuSetting},
...
pub(crate) struct ApplicationInner {
  config_registry: HetuConfigRegistry,
...
impl Display for Application {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    write!(f, "Application({}|{})", self.hetu_setting().app().name(), self.0.start_time)
  }
}
...
impl Application {
  ...
  pub fn config_registry(&self) -> &HetuConfigRegistry {
  ...
  pub fn hetu_setting(&self) -> Arc<HetuSetting> {
    self.0.config_registry.hetu_setting()
  }
...
impl ApplicationBuilder {
  pub fn get_hetu_config(&self) -> Arc<HetuSetting> {
    self.config_registry.hetu_setting()
  }

  pub fn with_config_registry(&mut self, config_registry: HetuConfigRegistry) -> &Self {
  ...
  fn build_application(&mut self) -> Application {
    ...
    let init_time = configuration_state.hetu_setting().app().time_now();
...
#[cfg(test)]
mod tests {
  use hetu_common::env::remove_envs;
  ...
  async fn test_application_run() {
    remove_envs(&[
      "HETU__APP__NAME",
      "HETU__WEB__SERVER_ADDR",
      "HETU__SECURITY__TOKEN__SECRET_KEY",
      "HETU__SECURITY__PWD__PWD_KEY",
    ])
    .unwrap();
    Application::builder().run().await.unwrap();
    let app = Application::global();
    assert_eq!(app.hetu_setting().app().name(), "hetu");
  }
}
