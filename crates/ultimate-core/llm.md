# fusion-core LLM 使用指南

> fusion-core 是 Fusion-Data 项目的核心库，提供 Application 容器、依赖注入、配置管理、安全工具等核心功能。
...
```rust
use ultimate_core::{Application, ApplicationBuilder, DataError, Builder};
...
```rust
use ultimate_core::{component::Component, DataError, application::ApplicationBuilder};
...
```rust
use ultimate_core::component::{ComponentArc, DynComponentArc};
...
```rust
use ultimate_core::{component::ComponentInstaller, Component, application::ApplicationBuilder, submit_component};

pub struct MyComponentInstaller;

impl ComponentInstaller for MyComponentInstaller {
  fn dependencies(&self) -> Vec<&str> {
    vec!["ultimate_core::configuration::AppSetting"]
  }
...
```rust
use ultimate_core::{plugin::Plugin, application::ApplicationBuilder, async_trait};
...
```rust
use ultimate_core::configuration::FusionSetting;

let ultimate_setting = app.ultimate_setting();
let app_setting = ultimate_setting.app();
let security_setting = ultimate_setting.security();
let log_setting = ultimate_setting.log();
```
...
```rust
use ultimate_core::security::jose::{encode_jwt_hs256, decode_jwt_hs256, JwtPayload, JwsHeader};
...
```rust
use ultimate_core::security::jose::{encode_jwt_es256, decode_jwt_es256};
...
```rust
use ultimate_core::security::jose::{encrypt_jwe_dir, decrypt_jwe_dir};
...
```rust
use ultimate_core::security::pwd::{generate_pwd, verify_pwd, is_strong_password};
...
```rust
use ultimate_core::security::RECOMMENDED_LENGTH; // 16 字节
```
...
```rust
use ultimate_core::utils::wait_exit_signals;
...
```rust
use ultimate_core::concurrent::{ServiceTask, TaskServiceHandle, TaskResult};
...
```rust
use ultimate_core::logforth::LogforthPlugin;
