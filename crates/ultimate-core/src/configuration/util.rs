...
pub fn load_config_with(custom_config: Option<Config>) -> ConfigureResult<Config> {
  ...
  if let Ok(file) = get_env("ULTIMATE_CONFIG_FILE") {
    log::info!("Load config from env ULTIMATE_CONFIG_FILE: {}", file);
    let path = Path::new(&file);
    if path.exists() {
      log::info!("ULTIMATE_CONFIG_FILE exists, load config from path: {}", path.display());
