...
mod tests {
  ...
  fn test_config_load() {
    ...
    set_env("ULTIMATE__APP__NAME", "ultimate-test").unwrap();

    let c = load_config().unwrap();
    println!("Config cache: {}", c.cache);
    let qc: UltimateSetting = c.get("ultimate").unwrap();

    assert_eq!(qc.security().token().secret_key(), b"8462b1ec9af827ebed13926f8f1e5409774fa1a21a1c8f726a4a34cf7dcabaf2");

    // 由环境变量 ULTIMATE__APP__NAME 提供
    assert_eq!(qc.app().name(), "ultimate-test");
