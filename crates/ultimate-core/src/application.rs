...
use ultimate_common::ahash::HashSet;
...
use crate::{
  ...
  configuration::{ConfigRegistry, Configurable, ConfigureResult, UltimateConfigRegistry, UltimateSetting},
...
impl Display for Application {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    write!(f, "Application({}|{})", self.ultimate_setting().app().name(), self.0.start_time)
  }
}
...
impl Application {
  ...
  pub fn config_registry(&self) -> &UltimateConfigRegistry {
  ...
  pub fn ultimate_setting(&self) -> Arc<UltimateSetting> {
    self.0.config_registry.ultimate_setting()
  }
...
impl ApplicationBuilder {
  pub fn get_ultimate_config(&self) -> Arc<UltimateSetting> {
    self.config_registry.ultimate_setting()
  }

  pub fn with_config_registry(&mut self, config_registry: UltimateConfigRegistry) -> &Self {
  ...
  fn build_application(&mut self) -> Application {
    ...
    let init_time = configuration_state.ultimate_setting().app().time_now();
...
#[cfg(test)]
mod tests {
  use ultimate_common::env::remove_envs;
  ...
  async fn test_application_run() {
    remove_envs(&[
      "ULTIMATE__APP__NAME",
      "ULTIMATE__WEB__SERVER_ADDR",
      "ULTIMATE__SECURITY__TOKEN__SECRET_KEY",
      "ULTIMATE__SECURITY__PWD__PWD_KEY",
    ])
    .unwrap();
    Application::builder().run().await.unwrap();
    let app = Application::global();
    assert_eq!(app.ultimate_setting().app().name(), "ultimate");
  }
}
