...
SecurityUtils 提供统一的 JWT/JWE 加解密能力，配合 `ultimate_web::extract_ctx` 在 Web Handler 中快速提取 `Ctx`（包装 `CtxPayload`）。
...
```rust
use ultimate_core::security::SecurityUtils;
use ultimates::common::ctx::CtxPayload;
...
`ultimate_web::extract_ctx(parts, security)` 会从 `Authorization: Bearer` 或查询参数 `access_token` 获取 token，调用 `SecurityUtils::decrypt_jwt(security.pwd(), token)` 解密并返回 `Ctx`。建议在 Handler 或 Service 构造中复用该方法，以统一令牌解析逻辑。
