# hetu-common LLM 使用指南

> hetu-common 是 Fusion-Data 项目的通用工具库，提供错误处理、上下文管理、加密、UUID 等基础功能。
...
```rust
use hetus::common::digest::{sha256, sha256_string};
...
```rust
use hetus::common::digest::{hmac_sha256, hmac_sha256_string};
...
```rust
use hetus::common::digest::{b64u_encode, b64u_decode, b64u_decode_to_string};
...
```rust
use hetus::common::uuid::ExtraUuid;
...
```rust
use hetus::common::uuid::ExtraBase64;
...
```rust
use hetus::common::time::{now, now_utc, now_offset, now_epoch_millis, now_epoch_seconds};
...
```rust
use hetus::common::time::{parse_utc, format_time, utc_from_millis, datetime_from_millis};
...
```rust
use hetus::common::time::{DateTime, Duration, FixedOffset, Local, Utc, OffsetDateTime};
```
...
```rust
use hetus::common::model::sensitive::UriString;
...
```rust
use hetus::common::helper::{default_bool_true, default_bool_false, default_i64_0, default_i64_1};
