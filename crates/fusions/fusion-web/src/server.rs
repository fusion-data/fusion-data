...
pub async fn init_server(router: Router) -> Result<(), DataError> {
  ...
  init_server_with_config(&conf, router, None).await
}

pub async fn init_server_with_config(
  conf: &WebConfig,
  router: Router,
  ...
  let serve = if conf.enable_remote_addr {
    let s = axum::serve(listener, router.into_make_service_with_connect_info::<SocketAddr>());
    ...
    let s = axum::serve(listener, router.into_make_service());
    if let Some(shutdown_rx) = shutdown_rx {
      s.with_graceful_shutdown(shutdown_rx.is_shutdown_owned()).await
    } else {
      s.await
