...
// static BASE_PACKAGE: &str = ".fusion_flow.v1";
...
fn main() {
  ...
  b = b.file_descriptor_set_path(out_dir.join("fusion_flow_descriptor.bin"))
    // .message_attribute(BASE_PACKAGE, MESSAGE_ATTR)
    // .bytes(["."])
    .extern_path(".ultimate_api.v1", "::ultimate_api::v1")
    .extern_path(".fusion_flow_api.v1", "::fusion_flow_api::v1");
  ...
  b.compile_protos(
    &["proto/fusion_flow/v1/flow.proto"],
    &["proto", "../fusion-flow-api/proto", "../../ultimates/ultimate-api/proto"],
  )
  .unwrap();
}
