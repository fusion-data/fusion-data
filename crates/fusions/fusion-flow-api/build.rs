...
fn main() {
  ...
  let mut iam_b = tonic_build::configure()
    ...
    // .bytes(["."])
    .file_descriptor_set_path(out_dir.join("fusion_flow_api_descriptor.bin"));
  ...
  iam_b
    .compile_protos(&["proto/fusion_flow_api/v1/flow_api.proto"], &["proto", "../../ultimates/ultimate-api/proto"])
    .unwrap();
}
