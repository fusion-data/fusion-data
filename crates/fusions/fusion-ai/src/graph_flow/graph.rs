...
impl Graph {
  ...
  pub fn add_edge(&self, from: impl Into<String>, to: impl Into<String>) -> &Self {
    self.edges.lock().unwrap().push(Edge { from: from.into(), to: to.into(), condition: None });
    self
  }
    ...
    F: Fn(&Context) -> bool + Send + Sync + 'static,
  {
    ...
    // "yes" branch
    edges.push(Edge { from: from.clone(), to: yes_to, condition: Some(predicate) });

    // "else" branch (unconditional fallback)
    edges.push(Edge { from, to: no_to, condition: None });
...
impl GraphBuilder {
  ...
  pub fn build(self) -> Graph {
    ...
    if task_count > 1 {
      ...
      let all_task_ids: Vec<String> = self.graph.tasks.iter().map(|t| t.key().clone()).collect();
