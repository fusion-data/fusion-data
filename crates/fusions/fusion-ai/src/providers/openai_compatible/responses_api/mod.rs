          ...
          rig::message::UserContent::ToolResult(rig::completion::message::ToolResult {
            ...
            for tool_result_content in tool_content {
              ...
              items.push(InputItem {
                role: None,
                input: InputContent::FunctionCallOutput(ToolResult {
                  call_id: call_id.clone().expect("The call ID of this tool should exist!"),
                  output: text,
                  status: ToolStatus::Completed,
                }),
              });
    ...
    rig::completion::Message::Assistant { id, content } => {
      ...
      for assistant_content in content {
        match assistant_content {
          ...
          rig::message::AssistantContent::ToolCall(rig::message::ToolCall { id: tool_id, call_id, function }) => {
            items.push(InputItem {
              role: None,
  ...
  fn try_from((model, req): (String, rig::completion::CompletionRequest)) -> Result<Self, Self::Error> {
    let input = {
      ...
      full_history.extend(
        partial_history
          .into_iter()
          .map(|x| try_from_message_to_vec_input_item(x).unwrap())
    ...
    let input = OneOrMany::many(input).expect("This should never panic - if it does, please file a bug report");
    ...
    let additional_parameters = if let Some(map) = req.additional_params {
      serde_json::from_value::<AdditionalParameters>(map).expect(
        "Converting additional parameters to AdditionalParameters should never fail as every field is an Option",
      )
    } else {
...
impl AdditionalParameters {
  pub fn to_json(self) -> serde_json::Value {
    serde_json::to_value(self)
      .expect("this should never fail since a struct that impls Deserialize will always be valid JSON")
  }
}
  ...
  T: HttpClientExt + Clone + std::fmt::Debug + Default + Send + 'static,
{
  ...
  async fn completion(
    ...
    span.record(
      ...
      serde_json::to_string(&request.input).expect("openai request to successfully turn into a JSON value"),
    );
...
impl From<AssistantContent> for completion::AssistantContent {
  fn from(value: AssistantContent) -> Self {
    match value {
      AssistantContent::Refusal { refusal } => completion::AssistantContent::Text(Text { text: refusal }),
      AssistantContent::OutputText(Text { text }) => completion::AssistantContent::Text(Text { text }),
    }
  }
}
...
pub fn try_from(message: message::Message) -> Result<Vec<Message>, message::MessageError> {
  match message {
    ...
    message::Message::Assistant { content, id } => {
      ...
      match content.first() {
        rig::message::AssistantContent::Text(Text { text }) => Ok(vec![Message::Assistant {
