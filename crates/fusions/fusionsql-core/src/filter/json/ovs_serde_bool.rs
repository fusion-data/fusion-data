...
use crate::filter::{Error, OpValBool, OpValBool};
...
impl<'de> Deserialize<'de> for OpValBool {
  fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
  where
    D: Deserializer<'de>,
  {
    deserializer.deserialize_any(BoolOpValVisitor)
  }
}

struct BoolOpValVisitor;

impl<'de> Visitor<'de> for BoolOpValVisitor {
  type Value = OpValBool; // for deserialize

  fn expecting(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
    write!(formatter, "BoolOpValVisitor visitor not implemented for this type.")
  }
