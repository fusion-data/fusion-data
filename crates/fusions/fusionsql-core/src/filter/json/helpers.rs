...
#[allow(dead_code)]
pub fn as_uuid(value: Value) -> Result<uuid::Uuid, Error> {
  if let Value::String(item) = value {
    Ok(uuid::Uuid::parse_str(item.as_str()).map_err(|_e| Error::JsonValNotOfType("Uuid"))?)
  } else {
...
#[cfg(feature = "with-uuid")]
#[allow(dead_code)]
pub fn into_uuids(value: Value) -> Result<Vec<uuid::Uuid>, Error> {
  let mut values = Vec::new();
  ...
  for value in array.into_iter() {
    if let Value::String(item) = value {
      values.push(
        uuid::Uuid::parse_str(item.as_str())
