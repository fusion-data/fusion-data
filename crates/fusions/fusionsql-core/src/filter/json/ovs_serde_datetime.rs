...
use crate::filter::{Error, OpValDateTime, OpValDateTime};

use super::OpValueToOpValType;

impl Serialize for OpValDateTime {
...
impl<'de> Deserialize<'de> for OpValDateTime {
  fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
  where
    D: serde::Deserializer<'de>,
  {
    deserializer.deserialize_any(DateTimeOpValVisitor)
  }
}

struct DateTimeOpValVisitor;

impl<'de> Visitor<'de> for DateTimeOpValVisitor {
  type Value = OpValDateTime;

  fn expecting(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
    write!(formatter, "DateTimeOpValVisitor visitor not implemented for this type.")
  }
