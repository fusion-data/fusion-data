...
use crate::filter::{Error, OpValUuid, OpValUuid};

impl Serialize for OpValUuid {
...
impl<'de> Deserialize<'de> for OpValUuid {
  fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
  where
    D: serde::Deserializer<'de>,
  {
    deserializer.deserialize_any(UuidOpValVisitor)
  }
}

struct UuidOpValVisitor;

impl<'de> Visitor<'de> for UuidOpValVisitor {
  type Value = OpValUuid;

  fn expecting(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
    write!(formatter, "UuidOpValVisitor visitor not implemented for this type.")
  }
