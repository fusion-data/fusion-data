...
pub fn derive_filter_nodes_inner(input: TokenStream) -> TokenStream {
    ...
    if type_name.starts_with("Option ") && type_name.contains("OpVal") {
      if let Some(ident) = field.ident.as_ref() {
        ...
        props_filter_node_options.push(quote! {
          fusion_sql::filter::FilterNodeOptions {
        ...
        let block = if let Some(to_sea_condition_fn) = modql_field_attr.to_sea_condition_fn {
          ...
          quote! {
            // None
            let fn_holder = fusion_sql::filter::ToSeaConditionFnHolder::new(#to_sea_condition_fn);
          ...
          quote! {
            // None
            let fn_holder = fusion_sql::filter::ToSeaValueFnHolder::new(#to_sea_value_fn);
            let fn_holder = Some(fn_holder.into());
          }
        } else {
          quote! {
  ...
  let out_impl_into_filter_nodes = quote! {
    impl fusion_sql::filter::IntoFilterNodes for #struct_name {
      fn filter_nodes(self, rel: Option<String>) -> Vec<fusion_sql::filter::FilterNode> {
  ...
  let out_into_filter_node = quote! {
    impl From<#struct_name> for Vec<fusion_sql::filter::FilterNode> {
      fn from(val: #struct_name) -> Self {
        fusion_sql::filter::IntoFilterNodes::filter_nodes(val, None)
      }
    }
  };

  let out_into_op_group = quote! {
    impl From<#struct_name> for fusion_sql::filter::FilterGroup {
      fn from(val: #struct_name) -> Self {
        let nodes: Vec<fusion_sql::filter::FilterNode> = val.into();
        nodes.into()
      }
    }
  };
  ...
  let out_into_op_groups = quote! {
    impl From<#struct_name> for fusion_sql::filter::FilterGroups {
      fn from(val: #struct_name) -> Self {
        let nodes: Vec<fusion_sql::filter::FilterNode> = val.into();
        nodes.into()
      }
  ...
  let out_sea_filter = quote! {
    impl TryFrom<#struct_name> for sea_query::Condition {
      type Error = fusion_sql::filter::IntoSeaError;

      fn try_from(val: #struct_name) -> fusion_sql::filter::SeaResult<Self> {
        fusion_sql::filter::FilterGroup::from(val).try_into()
      }
    }
  };
