...
fn impl_has_fields(
  ...
  let props_field_refs = field_props.iter().map(|field_prop| {
    ...
    quote! {&fusion_sql::field::FieldRef{rel: #rel, name: #name}}
  });
  ...
  let props_field_metas = field_props.iter().map(|field_prop| {
    ...
    quote! {&fusion_sql::field::FieldMeta{
        rel: #rel,
  ...
  let output = quote! {
    ...
    impl fusion_sql::field::HasFields for #struct_name {
      ...
      fn field_refs() -> &'static [&'static fusion_sql::field::FieldRef] {
        &[#(
        #props_field_refs,
        )*]
      }

      fn field_metas() -> &'static fusion_sql::field::FieldMetas {
        static METAS: &[&fusion_sql::field::FieldMeta] = &[#(
        #props_field_metas,
        )*];

        static METAS_HOLDER: fusion_sql::field::FieldMetas = fusion_sql::field::FieldMetas::new(METAS);
...
fn impl_has_sea_fields(
  ...
  fn field_options_quote(mfield_prop: &ModelsqlFieldProp) -> proc_macro2::TokenStream {
    if let Some(cast_as) = &mfield_prop.cast_as {
      quote! { fusion_sql::field::FieldOptions { cast_as: Some(#cast_as.to_string()) } }
    } else {
      quote! { fusion_sql::field::FieldOptions { cast_as: None } }
    }
  }
  ...
  let all_fields_quotes = field_props.iter().map(|p| {
    ...
    quote! {
      ff.push(
        fusion_sql::field::SeaField::new_with_options(fusion_sql::SIden(#name), self.#ident.into(), #field_options_q)
      );
    }
  });
    ...
    if p.is_option {
      quote! {
        if let Some(val) = self.#ident {
          ff.push(
            fusion_sql::field::SeaField::new_with_options(fusion_sql::SIden(#name), val.into(), #field_options_q)
          );
        }
      }
    } else {
  ...
  let sea_fields_with_mask_quotes = field_props.iter().map(|p| {
    ...
    let create_field_expr = |value_expr: proc_macro2::TokenStream| {
      quote! {
        ff.push(
          fusion_sql::field::SeaField::new_with_options(fusion_sql::SIden(#name), #value_expr.into(), #field_options_q)
        );
      }
    };
  ...
  let output = quote! {
    ...
    impl fusion_sql::field::HasSeaFields for #struct_name {

      fn not_none_sea_fields(self) -> fusion_sql::field::SeaFields {
        let mut ff: Vec<fusion_sql::field::SeaField> = Vec::new();
        #(#not_none_fields_quotes)*
        fusion_sql::field::SeaFields::new(ff)
      }

      fn sea_fields_with_mask(self) -> fusion_sql::field::SeaFields {
        let mut ff: Vec<fusion_sql::field::SeaField> = Vec::new();
        #(#sea_fields_with_mask_quotes)*
        fusion_sql::field::SeaFields::new(ff)
      }

      fn all_sea_fields(self) -> fusion_sql::field::SeaFields {
        let mut ff: Vec<fusion_sql::field::SeaField> = Vec::new();
        #(#all_fields_quotes)*
        fusion_sql::field::SeaFields::new(ff)
      }
      ...
      fn sea_idens() -> Vec<sea_query::SeaRc<dyn sea_query::Iden>> {
        vec![#(
        sea_query::IntoIden::into_iden(fusion_sql::SIden(#prop_all_names)),
        )*]
      }
