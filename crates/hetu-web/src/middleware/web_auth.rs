...
use hetu_core::security::AccessToken;
  ...
  fn authorize(&mut self, request: http::Request<Self::RequestBody>) -> Self::Future {
    ...
    match self.external.as_ref() {
      ...
      None => Box::pin(async {
        ...
        let ctx = extract_ctx(&parts, Application::global().hetu_setting().security()).map_err(web_error_2_body)?;
        parts.extensions.insert(ctx);
        Ok(Request::from_parts(parts, body))
      }),
    }
...
async fn validate_token_remote(url: &str, client: &reqwest::Client, parts: &Parts) -> Result<Ctx, WebError> {
  ...
  if response.status().is_success() {
    ...
    let ctx_payload = hetu_common::ctx::CtxPayload::from(response_map);
