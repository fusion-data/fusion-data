...
use hetu_common::model::IdI64Result;
use hetu_common::time::now_offset;
use hetu_core::configuration::SecuritySetting;
use hetu_core::security::{AccessToken, SecurityUtils};
use hetu_core::utils::get_trace_id;
...
#[cfg(feature = "with-ulid")]
#[inline]
pub fn ok_ulid(id: Ulid) -> WebResult<hetu_common::model::IdUlidResult> {
  Ok(hetu_common::model::IdUlidResult::new(id).into())
}

#[inline]
pub fn ok_uuid(id: uuid::Uuid) -> WebResult<hetu_common::model::IdUuidResult> {
  Ok(hetu_common::model::IdUuidResult::new(id).into())
}
